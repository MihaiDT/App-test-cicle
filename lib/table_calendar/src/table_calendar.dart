...
class _TableCalendarState<T> extends State<TableCalendar<T>> {
  ...
  String getItalianMonth(DateTime dateTime) {
    final String monthText = DateFormat('MMMM', 'it')
        .format(DateTime(2023, dateTime.month))
        .toUpperCase();

    return monthText;
  }

  List<Widget> get weekDays {
    List<Widget> texts = [];
    for (final String element in CustomDateUtils.daysOfWeek) {
      texts.add(
        Text(
          element,
